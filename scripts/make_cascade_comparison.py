...
configs = [
    ...
    (
        "BERT + Gemma 31B + 10-shot (n=50)",
        "bert-consultant-fewshot10-gemma-n50",
        "D",
        "bert-integration",
    ),
]
...
def main() -> None:
    ...
    for label, dirname, marker, group in configs:
        ...
        ax.scatter(
            r1, sa, marker=marker, s=size, color=color, edgecolor="black", linewidth=0.7, zorder=3
        )
        ...
        ax.annotate(
            label,
            (r1, sa),
            xytext=(r1 + offset_x, sa + offset_y),
            fontsize=8.5,
            ha=ha,
            arrowprops=None,
        )
    ...
    ax.set_title(
        "Configurations tested on the SocratDataset test split (n=50 matched)\n"
    ...
    handles = [
        ...
        Line2D(
            ...
            linestyle="",
            markersize=9,
            color=colors_by_group["locked"],
            label="A3B locked (no improvement)",
        ),
        Line2D(
            [],
            [],
            ...
            linestyle="",
            markersize=9,
            color=colors_by_group["prompt-eng"],
            label="Prompt-engineering (10-shot)",
        ),
        Line2D(
            [],
            [],
            ...
            linestyle="",
            markersize=9,
            color=colors_by_group["bert-integration"],
            label="BERT consultant integration",
        ),
    ]
