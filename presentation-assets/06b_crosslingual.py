...
bars_pub = ax.bar(x - bar_w/2, published_ZH, bar_w,
                  color=ps.SLATE, alpha=0.85,
                  edgecolor=ps.NAVY, linewidth=0.9,
                  label="Chinese (paper baseline)  ·  GPT-4o consultant + SocratTeachLLM teacher")
bars_ours = ax.bar(x + bar_w/2, ours_EN, bar_w,
                   color=ps.RED, alpha=0.85,
                   edgecolor=ps.NAVY, linewidth=0.9,
...
for xv, v in zip(x - bar_w/2, published_ZH):
    ax.text(xv, v + 1.5, f"{v:.2f}", ha="center", va="bottom",
...
def delta_label(ax, idx, delta, y, color=ps.NAVY):
    ...
    ax.text(idx, y + 1.4, f"Δ = {sign}{abs(delta):.2f}",
            ha="center", va="bottom",
...
fig.text(0.79, 0.55,
         ...
         ha="left", va="center", fontsize=12, color=ps.GOLD, fontweight="bold",
         bbox=dict(boxstyle="round,pad=0.55", facecolor=ps.LIGHT_GOLD,
                   edgecolor=ps.GOLD, linewidth=1.2))
