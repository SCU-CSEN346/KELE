...
for i, (code, name, n_states, n_turns) in enumerate(stages):
    ...
    ax.text(
        ...
        fontweight="bold",
        color="white",
        bbox=dict(boxstyle="round,pad=0.30", facecolor=edge, edgecolor="none"),
    )
    ...
    ax.text(
        ...
        bbox=dict(
            boxstyle="round,pad=0.28", facecolor=pct_face, edgecolor=pct_edge, linewidth=pct_lw
        ),
    )
    ...
    if i < len(stages) - 1:
        ...
        ax.annotate(
            ...
            arrowprops=dict(arrowstyle="-|>", color=ps.NAVY, lw=2.4, mutation_scale=26),
        )
