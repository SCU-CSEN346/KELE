...
def draw(ax, vals, ylabel, title, *, ymax):
    ...
    for x, v in zip(xs, vals):
        ax.text(x, v + ymax * 0.018, f"{v:.2f}", ha="center", va="bottom",
                fontsize=13, fontweight="bold", color=ps.NAVY)
    ax.set_xticks(xs)
...
draw(ax1, stage_bal, "Stage-balanced macro state acc (%)",
     "STL collapses on clean held-out data", ymax=92)
draw(ax2, rouge_1,   "ROUGE-1 (%)",
     "Surface form collapses with it", ymax=60)
...
ax1.annotate("",
             ...
             arrowprops=dict(arrowstyle="-|>", color=ps.GOLD, lw=2.4,
                             mutation_scale=22))
# Drop-label sits in a card centered above the arrow
ax1.text(0.5, arrow_y + 3.5, f"Δ = −{drop1:.2f} pp",
         ha="center", va="bottom", fontsize=14, fontweight="bold", color=ps.GOLD,
         bbox=dict(boxstyle="round,pad=0.5", facecolor=ps.LIGHT_GOLD,
