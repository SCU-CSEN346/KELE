...
# --- Header row ---
ax.text(X_RANK,  y_header, "#",            ha="left",  va="center", fontsize=11, fontweight="bold", color=ps.SLATE)
ax.text(X_LEVER, y_header, "Lever",        ha="left",  va="center", fontsize=11, fontweight="bold", color=ps.SLATE)
ax.text(X_BLURB, y_header, "What it does", ha="left",  va="center", fontsize=11, fontweight="bold", color=ps.SLATE)
ax.text(X_STATE, y_header, "State acc",    ha="right", va="center", fontsize=11, fontweight="bold", color=ps.SLATE)
ax.text((X_BAR_L + X_BAR_R) / 2, y_header, "(visual)", ha="center", va="center",
        fontsize=10, color=ps.GRAY, style="italic")
ax.text(X_R1,    y_header, "ROUGE-1",      ha="right", va="center", fontsize=11, fontweight="bold", color=ps.SLATE)
ax.text(X_BLEU,  y_header, "BLEU-4",       ha="right", va="center", fontsize=11, fontweight="bold", color=ps.SLATE)
...
for i, (lever, blurb, state, r1, bleu) in enumerate(levers):
    ...
    # Rank
    rank_str = f"{i+1}"
    ax.text(X_RANK, y, rank_str, ha="left", va="center",
            fontsize=14, fontweight="bold", color=row_color)

    # Lever name. Row highlight + bold red text already signal the winner.
    ax.text(X_LEVER, y, lever, ha="left", va="center",
            fontsize=13.5, fontweight=row_weight, color=row_color)

    # Blurb (one-line description)
    ax.text(X_BLURB, y, blurb, ha="left", va="center",
            fontsize=11.5, color=ps.SLATE, style="italic")

    # State acc (right-aligned)
    ax.text(X_STATE, y, f"{state:.2f}", ha="right", va="center",
            fontsize=13.5, fontweight="bold", color=row_color)
    ...
    ax.text(X_R1, y, f"{r1:.2f}", ha="right", va="center",
            fontsize=12.5, color=row_color)

    # BLEU-4 (right-aligned)
    ax.text(X_BLEU, y, f"{bleu:.2f}", ha="right", va="center",
            fontsize=12.5, color=row_color)
...
ax.text(
    ...
    ha="center", va="center",
    fontsize=12, fontweight="bold", color=ps.RED, style="italic",
)
