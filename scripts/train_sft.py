...
def main() -> None:
    ...
    class HFCheckpointCallback(TrainerCallback):
        ...
        TRAIN_HF_PUSH_EVERY (default 50 steps).  Runs in a daemon thread
        so it never blocks the training loop; if the previous push is still
        in-flight when the next one fires, the new push is skipped and logged.
        Also fires unconditionally at on_train_end to capture the final state.
        """
