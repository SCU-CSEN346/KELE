...
All dataset content, annotation methodology, and Socratic teaching framework are the intellectual property of the original authors.

## Source Data

The dataset is derived from real Chinese primary school exam questions, manually annotated with Socratic-style tutoring dialogues. Original data is distributed via:
...
No modifications were made to the dataset content. The upload script reshapes the original JSONL files into HuggingFace Dataset format with `id` and `conversations` fields.
