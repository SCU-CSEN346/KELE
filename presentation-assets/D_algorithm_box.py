...
ax.text(7.75, 8.55,
        ...
        ha="center", va="center", fontsize=20, fontweight="bold", color=ps.NAVY)
ax.text(7.75, 8.05,
...
ax.plot([card_x + 0.30, card_x + card_w - 0.30],
        [y_div, y_div], color=BLACK, linewidth=1.2, alpha=0.45)
...
for i, (lineno, code, comment) in enumerate(steps):
    y = y0 - i * dy
    ax.text(x_lineno, y, lineno,
            ha="left", va="center", fontsize=12, fontweight="bold",
            color=BLACK, family="monospace", alpha=0.65)
    ax.text(x_code, y, code,
            ha="left", va="center", fontsize=12,
            color=BLACK, family="monospace", fontweight="medium")
    if comment:
        ax.text(x_comment, y, "// " + comment,
                ha="left", va="center", fontsize=11,
                color=BLACK, style="italic", family="monospace", alpha=0.80)

# --- Footer callout ---
ax.text(7.75, 0.30,
        "Line 1 replaces the LLM-consultant call  ·  Line 4 replaces JSON-schema routing",
        ha="center", va="center", fontsize=12, fontweight="bold", color=ps.GOLD)
