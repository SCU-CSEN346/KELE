...
for i, c in enumerate(consultants):
    ...
    ax.bar(i + offsets["base_state"], base_state, bar_w,
           color=ps.NAVY, edgecolor=ps.NAVY, linewidth=0.8,
           label="GLM-base · State acc" if i == 0 else None)
    ax.bar(i + offsets["stl_state"], stl_state, bar_w,
           color=ps.RED, alpha=0.55, edgecolor=ps.NAVY, linewidth=0.8,
           label="STL · State acc" if i == 0 else None)
    ax.bar(i + offsets["base_r1"], base_r1, bar_w,
           color=ps.NAVY, edgecolor=ps.NAVY, linewidth=0.8, hatch="//",
           label="GLM-base · ROUGE-1" if i == 0 else None)
    ax.bar(i + offsets["stl_r1"], stl_r1, bar_w,
           color=ps.RED, edgecolor=ps.NAVY, linewidth=0.8, hatch="//",
           label="STL · ROUGE-1" if i == 0 else None)

    # value labels (just above each bar top)
    for off_key, val in [("base_state", base_state), ("stl_state", stl_state),
                          ("base_r1", base_r1), ("stl_r1", stl_r1)]:
        ax.text(i + offsets[off_key], val + 0.6, f"{val:.1f}",
                ha="center", va="bottom", fontsize=10, color=ps.NAVY, fontweight="medium")
    ...
    ax.annotate("", xy=(dx_right, bracket_y_state), xytext=(dx_left, bracket_y_state),
                arrowprops=dict(arrowstyle="<->", color=ps.NAVY, lw=1.4))
    d_state = stl_state - base_state
    ax.text((dx_left + dx_right) / 2, bracket_y_state + 1.4,
            f"Δstate  +{d_state:.2f} pp",
            ha="center", va="bottom", fontsize=11, color=ps.NAVY, fontweight="bold")
    ...
    ax.annotate("", xy=(dx_right, bracket_y_r1), xytext=(dx_left, bracket_y_r1),
                arrowprops=dict(arrowstyle="<->", color=ps.AMBER, lw=1.7))
    d_r1 = stl_r1 - base_r1
    ratio = d_r1 / d_state
    ax.text((dx_left + dx_right) / 2, bracket_y_r1 + 1.4,
            f"ΔR-1  +{d_r1:.2f} pp   ({ratio:.2f}x)",
            ha="center", va="bottom", fontsize=11, color=ps.GOLD, fontweight="bold")
...
ax.set_xticklabels([f"Consultant: {_CONSULTANT_LABEL[c]}" for c in consultants],
...
fig.text(0.97, 0.88,
         ...
         ha="right", va="top", fontsize=11, color=ps.GOLD, fontweight="bold",
         bbox=dict(boxstyle="round,pad=0.55", facecolor=ps.LIGHT_GOLD,
                   edgecolor=ps.GOLD, linewidth=1.3))

# Legend at the bottom, 4-col, frameless
ax.legend(loc="lower center", bbox_to_anchor=(0.5, -0.18),
          fontsize=11, ncol=4, frameon=False)
