...
for i, (g, o) in enumerate(zip(gpt4o, ours)):
    ...
    ax.text(
        ...
        bbox=dict(
            ...
            edgecolor=color,
            linewidth=1.2 if is_dominant else 0.8,
        ),
    )
...
for i in range(len(stages)):
    ax.plot(
        [i, i],
        [max(gpt4o[i], ours[i]) + 3.5, ANNOT_Y - 6],
        color=ps.DIVIDER,
        linewidth=0.8,
        linestyle=":",
        zorder=0,
...
ax.set_xticks(xs)
ax.set_xticklabels([f"{code}\n{name}" for code, name in stages], fontsize=13, fontweight="medium")
...
ax.legend(loc="lower center", bbox_to_anchor=(0.5, -0.32), fontsize=12, frameon=False, ncol=2)
...
ax.text(
    ...
    fontweight="bold",
    color=ps.RED,
    bbox=dict(boxstyle="round,pad=0.60", facecolor=ps.LIGHT_RED, edgecolor=ps.RED, linewidth=1.5),
)
