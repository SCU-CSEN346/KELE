...
7. **IN PROGRESS (3 of 4 cells done) — Full-test-set ($n{=}681$) local-vs-frontier comparison at parity, NOW INVERTED TO OVERTAKING.** What had been a 2026-05-23 screening-tier "1.12-pt parity gap" claim was reframed by canonical-scale data: cells #3 and #4a (2026-05-25) widened the gap to 1.41–3.35 pts behind frontier, then cell #2 (2026-05-26) **inverted the relationship entirely**. `qwen3.5 × Gemma-31B · fewshot10 · n=681` lands at unified **72.24**, **#1 master**, beating the frontier ceiling (`bert × Claude-Sonnet · top3 · n=681`, 70.06) by **+2.18 unified pts** at canonical sample size. The four targeted $n{=}681$ cells:
   - `bert-fixed × Gemma-31B · fewshot10 · n=681` — **pending** — clean re-baseline of the legacy locked headline (~12 GPU-h, ~$15). Lower priority now — the overtaking claim is locked by cell #2; cell #1 only confirms the duplication-artifact magnitude at canonical scale.
   - `qwen3.5 × Gemma-31B · fewshot10 · n=681` — ✅ **DONE 2026-05-26** — landed at unified **72.24** (**#1 master**; 61.32 stage_bal / 8.32 judge). Walltime ~2h eval (4 crashes resolved en route, see EXPERIMENT_LOG) + 21.5 min judge, judge $15.70. Output at `results/t4-bert-gemma-fewshot10-n681/`. **Overtakes frontier by +2.18 unified pts; overtakes legacy locked headline by +3.59 pts; positive scaling +3.30 pts vs n=50 baseline.** Per-stage: a=100 b=46.39 c=35.42 d=46.36 e=78.45.
   - `qwen3.5 × A3B-35B · fewshot10 · n=681` — ✅ **DONE 2026-05-25** — landed at unified **67.81** (#12 master; 60.02 stage_bal / 7.56 judge). Walltime 9h41m, judge $16.53. Output at `results/t4-bert-a3b-fewshot10-n681/`.
   - `qwen3.5 × Qwen-27B · no-think · fewshot10 · n=681` — ✅ **DONE 2026-05-25** — landed at unified **66.71** (#17 master; 58.16 stage_bal / 7.53 judge). Walltime 64 min eval + 18 min judge, judge $16.22. Output at `results/t4-bert-qwen27b-nothink-fewshot10-n681/`. Zero schema fallbacks across 4010 turns. The think-mode variant remains contingent on the CUDA-launch-timeout mitigation (see `memory/feedback_qwen27b_context_cap.md`); no-think is the safe substitute.

   Three of four cells now anchor a canonical-scale local sub-leaderboard. **The "parity" framing has been superseded by overtaking** — see [`memory/local_frontier_parity_2026_05_23.md`](../memory/local_frontier_parity_2026_05_23.md) and the 2026-05-26 EXPERIMENT_LOG entry. Remaining budget: ~12 GPU-h + ~$15 API for cell #1 (`bert-fixed × Gemma-31B`), which confirms the duplication-artifact magnitude at canonical scale but no longer load-bearing for the headline. All cells launched with `KELE_PARALLEL_WORKERS=4` (server `-np` updated to 4 in `serve_gemma4_31b.sh` 2026-05-26 — matches worker count to prevent the LCP-router slot-spread memory inflation observed during cell #2's crashes).
