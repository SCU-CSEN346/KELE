...
7. **IN PROGRESS (2 of 4 cells done) — Full-test-set ($n{=}681$) local-vs-frontier comparison at parity.** The local–frontier parity finding (best honest open-weight 68.94 unified vs.\ best frontier 70.06 unified; 1.12-pt gap; documented in `docs/UNIFIED_RANKING.md` and `docs/EXPERIMENT_LOG.md` 2026-05-23) rests on $n{=}50$ post-fix data + the legacy pre-fix `bert` $n{=}681$ headline. The asymmetric sample-size coverage means we cannot yet make an apples-to-apples local-vs-frontier statement at canonical sample size. The four targeted $n{=}681$ cells:
   - `bert-fixed × Gemma-31B · fewshot10 · n=681` — **pending** — clean re-baseline of the locked headline (~12 GPU-h, ~$15)
   - `qwen3.5 × Gemma-31B · fewshot10 · n=681` — **pending** — current cross-teacher winner at full sample (~13 GPU-h, ~$15)
   - `qwen3.5 × A3B-35B · fewshot10 · n=681` — ✅ **DONE 2026-05-25** — landed at unified **67.81** (#11 master; 60.02 stage_bal / 7.56 judge). Walltime 9h41m, judge $16.53. Output at `results/t4-bert-a3b-fewshot10-n681/`. Canonical-scale parity gap: 2.25 unified pts behind frontier (1.41 pts via the legacy locked-headline anchor).
   - `qwen3.5 × Qwen-27B · no-think · fewshot10 · n=681` — ✅ **DONE 2026-05-25** — landed at unified **66.71** (#16 master; 58.16 stage_bal / 7.53 judge). Walltime 64 min eval + 18 min judge, judge $16.22. Output at `results/t4-bert-qwen27b-nothink-fewshot10-n681/`. Zero schema fallbacks across 4010 turns. Canonical-scale parity gap: 3.35 unified pts behind frontier — widest of the three honest n=681 cells measured so far. The think-mode variant remains contingent on the CUDA-launch-timeout mitigation (see `memory/feedback_qwen27b_context_cap.md`); no-think is the safe substitute.

   Compare against the existing `bert × Claude-Sonnet · top3 · n=681` (unified 70.06) and `bert × Claude-Opus · top3 · n=681` (unified 69.37). Each cell needs both `metrics_summary.json` and `judge_summary.json` to land on the unified leaderboard. Remaining budget: ~25 GPU-h + ~$30 API across the two pending Gemma cells. **This produces the full-sample-size local sub-leaderboard the paper needs to confirm parity definitively** (rather than as a screening-tier finding). Both pending cells should use `KELE_PARALLEL_WORKERS=4` (server -np already 6) to halve walltime.
