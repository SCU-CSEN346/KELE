...
CONFIGS = [
    ...
    ("GPT-4o + SocratTeachLLM (paper baseline)", None,
     {"state": 25.94, "r1": 44.61, "r2": 26.04, "rl": 38.02, "b4": 19.60, "n_turns": 4294, "n_dialogues": 681}),
    ...
    ("Gemma 4 31B + 10-shot (n=50 ref)", None,
     {"state": 51.06, "r1": 38.53, "r2": 16.93, "rl": 29.48, "b4": 9.68, "n_turns": 284, "n_dialogues": 50}),
    ...
    ("Sonnet 4.6 consultant + SocratTeachLLM (n=50)", "results/claude-sonnet-consultant-socratteachllm-n50", None),
    ("Sonnet 4.6 consultant + SocratTeachLLM (n=50 clean rerun)", "results/claude-sonnet-consultant-socratteachllm-n50-clean", None),
    ("Opus 4.6 consultant + SocratTeachLLM (n=50)", "results/claude-opus-consultant-socratteachllm-n50", None),

    # Cross-lingual (SocratDataset-EN)
    ("Sonnet 4.6 consultant + SocratTeachLLM (EN)", "results/claude-sonnet-consultant-socratteachllm-EN-n50", None),
    ("Opus 4.6 consultant + SocratTeachLLM (EN)", "results/claude-opus-consultant-socratteachllm-EN-n50", None),
...
def collect_one(name: str, results_dir: str | None, hardcoded: dict | None) -> dict:
    ...
    if hardcoded:
        row.update({
            ...
            "surface_sum": (hardcoded.get("r1") or 0) + (hardcoded.get("r2") or 0) + (hardcoded.get("b4") or 0),
            "composite_state_r1": (hardcoded.get("state") or 0) + 0.5 * (hardcoded.get("r1") or 0),
            "semantic_r1": None,
            "judge_overall": None,
        })
        return row
...
def main():
    ...
    Path("results/master_leaderboard.json").write_text(json.dumps(out, indent=2, ensure_ascii=False))

    # Markdown table
    md = ["# Master leaderboard\n"]
    md.append(f"Configs available: {out['summary']['n_configs_available']}/{out['summary']['n_configs_total']}\n")

    md.append("\n## Sorted by composite (state + 0.5*R-1)\n")
    md.append("| Config | State | R-1 | R-2 | BLEU-4 | Semantic R-1 | LLM-judge | Composite | n_turns |")
    md.append("|---|---:|---:|---:|---:|---:|---:|---:|---:|")
    ...
    for r in avail:
        ...
        md.append(f"| {r['name']} | {r['state']:.2f} | {r['r1']:.2f} | {r['r2']:.2f} | {r['b4']:.2f} | {sem} | {judge} | {r['composite_state_r1']:.2f} | {r['n_turns']} |")
    ...
    for r in avail2:
        sem = f"{r['semantic_r1']:.4f}" if r['semantic_r1'] is not None else "—"
        judge = f"{r['judge_overall']:.2f}" if r['judge_overall'] is not None else "—"
        md.append(f"| {r['name']} | {r['r1']:.2f} | {r['r2']:.2f} | {r['b4']:.2f} | {r['surface_sum']:.2f} | {r['state']:.2f} | {judge} | {sem} |")
    ...
    if judged:
        ...
        for r in judged:
            sem = f"{r['semantic_r1']:.4f}" if r['semantic_r1'] is not None else "—"
            md.append(f"| {r['name']} | {r['judge_overall']:.2f} | {r['state']:.2f} | {r['r1']:.2f} | {sem} | {r['surface_sum']:.2f} |")
