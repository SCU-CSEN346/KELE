...
| ⏸ **Eval BLOCKED 2026-05-28 PM — moved to documented future work.** Attempted eval on n=681 with the freshly-quantized GGUF; SFT model produces degenerate repetition loops under the inference pipeline's prompt shape. Decisive diagnostic: same model + same server emits clean Socratic question (20 tokens, EOS, 0.5 s) when given verbatim training-format input, vs 2048-token repetition loop when given inference-format input. **Root cause: train/serve schema mismatch** — `dataset.py` emits multi-turn user/assistant pairs per dialogue turn; `socratic_teaching_system.py:445–453` emits a single user message with stringified history (`历史对话记录:` + `当前学生输入:` labels never seen in training). Eval row reframed as future work — pick Option A (re-prep training data to match inference shape; isolated to SFT track) or Option B (modify inference pipeline; would re-baseline every existing Tables 6/14 cell). Full diagnostic in `docs/EXPERIMENT_LOG.md` 2026-05-28 (PM) entry. Per the outcome matrix below (line 342), this is the "data-format issue" branch — locked headline at unified 72.24 stands. | — (deferred) | future work | Documented in §`sec:nextsteps` of `acl_latex.tex` and EXPERIMENT_LOG 2026-05-28 (PM). |
| Aggregate via `scripts/backtest_stage_balanced.py`; write paper paragraph (~150 words) + table rows in Tables 6 and 14. Decompose: master-ranking position from n=681; contamination signature from n=75 vs n=681 delta. | ~2 h human | deferred — eval blocked above | SFT row deferred from `acl_latex.tex`; locked-headline row at unified 72.24 unchanged. |
