...
## How KELE Measures Teaching

The KELE paper evaluates Socratic teaching across **13 metrics in three families**: 4 text-overlap metrics borrowed from machine translation, plus a 9-dimension pedagogical-quality framework the KELE authors defined (4 single-turn + 5 multi-turn).

### Family 1 — Surface-form / text-overlap (4 metrics)

Standard metrics borrowed from machine translation and summarization.

| Metric | What it measures |
|---|---|
| **ROUGE-1** | Unigram (single-word) overlap F1 against ground-truth reference |
| **ROUGE-2** | Bigram (word-pair) overlap F1 |
| **ROUGE-L** | Longest common subsequence F1 |
| **BLEU-4** | 4-gram precision |

- **Reward signal:** outputs that share words and phrases with the reference text.
- **Strength:** cheap, deterministic, language-model-agnostic, reproducible across labs.
- **Limitation:** measures *phrasing match*, not pedagogical correctness. Designed for translation, where the valid-output space is small — but Socratic teaching has many pedagogically-equivalent valid responses per turn.

### Family 2 — KELE pedagogical, single-turn (4 metrics, binary yes/no per turn)

KELE-defined quality dimensions, one judgment per dialogue turn.

- **PRR (Problem Relevance Rate)** — Is the teacher's question relevant to the problem and the student's input?
- **NDAR (No Direct Answer Rate)** — Did the teacher avoid giving the answer directly? Good Socratic teachers don't.
- **SPR (Summary Pass Rate)** — Did the closure-stage summary cover the core knowledge points?
- **IAR (Instruction Adherence Rate)** — Did the teacher follow the consultant's SocRule instruction?

- **Reward signal:** pedagogically grounded teaching competencies — relevance, restraint, summarization, rule-following.
- **Strength:** these are real teaching skills the audience would intuitively grade.
- **Limitation:** ground-truth labels in the published paper are GPT-4-generated; a model trained to imitate GPT-4's conventions has structural advantage.

### Family 3 — KELE pedagogical, multi-turn (5 metrics, 5-point Likert)

KELE-defined dialogue-trajectory dimensions, one rating per full dialogue.

- **Guidance** — Are the teacher's questions effective at guiding active student thinking?
- **Logicality** — Is the question flow logically coherent across turns?
- **Flexibility** — Does the teacher adapt to student feedback?
- **Repetitiveness** — Are the questions diverse, or do they repeat? (lower repetitiveness is better)
- **Clarity** — Are the questions clearly expressed?

- **Reward signal:** dialogue-level dynamics that single-turn metrics miss.
- **Strength:** captures the multi-turn structure of Socratic teaching.
- **Limitation:** subjective; high LLM-judge variance; same GPT-4-as-judge bias.

### A foreshadow

The KELE paper reports SocratTeachLLM, a 9-billion-parameter fine-tune, **outperforming GPT-4o on every single one of these 13 metrics**. A 9B specialist beating a frontier model on every dimension. On its face: a remarkable result.

**Hold a question for slide 12:** what would a 9B specialist need to outperform frontier on every dimension? Genuine pedagogical capability — or something else?

<!--
SPEAKER NOTES (Slide 3, ~2 min):
This is what KELE uses to measure Socratic teaching, and it's worth understanding because the metrics shape the whole story of our project. Thirteen metrics total, in three families.

Family one is text-overlap — ROUGE-1, ROUGE-2, ROUGE-L, and BLEU-4. These are standard metrics borrowed from machine translation. They reward outputs whose words and phrases match the reference text. The strength is they're cheap, deterministic, and reproducible. You can compute them in a script with no LLM judge. The limitation is they don't measure pedagogy at all. They measure phrasing similarity. For Socratic teaching, where many pedagogically equivalent responses exist per turn, that's a problematic match.

Family two is KELE's pedagogical evaluation, single-turn — four metrics with binary yes/no judgments per turn. PRR, Problem Relevance Rate: is the teacher's question relevant to the problem? NDAR, No Direct Answer Rate: did the teacher avoid spoiling the answer? SPR, Summary Pass Rate: at closure, did the teacher cover the core knowledge points? IAR, Instruction Adherence Rate: did the teacher follow the consultant's SocRule instruction? These are real teaching competencies. The limitation is the ground-truth labels in the original paper were GPT-4-generated, so a model trained to imitate GPT-4's conventions has structural advantage.

Family three is multi-turn pedagogical evaluation — five 5-point Likert metrics applied to the full dialogue trajectory. Guidance, logicality, flexibility, repetitiveness, clarity. These capture dialogue-level dynamics — coherence, adaptation, diversity — that single-turn metrics miss. The limitation is they're subjective and have high LLM-judge variance, plus the same GPT-4-bias concern.

Here's the setup. The KELE paper reports SocratTeachLLM — a 9-billion-parameter fine-tune — outperforming GPT-4o on every single one of these 13 metrics. A 9B specialist beating a frontier model on every dimension. On its face, that's a remarkable result. Hold a question for slide 12: what would a 9B specialist need to do, to outperform a frontier model on every pedagogical dimension? Genuine pedagogical capability — or something else? We'll come back to this.
-->
...
- 🏗️ **Architecture — separate consultant + teacher, within VRAM.** KELE's canonical stack requires **two model deployments simultaneously**, and two 30B-class open-weight LLMs do not co-resident on 32 GB. We *tried* fusion (single-backbone — see slide 7) and it was the first locked headline, but we moved past it. **The consultant-teacher separation is what KELE got right architecturally**, and our final system keeps that separation by downsizing the consultant axis to a deterministic classifier (24M or ~800M params) that fits alongside the 31B teacher on the same GPU.
...
Second, the tournament. Thirteen popular open-weight backbones, all at n=50, all in no-think mode. The no-think was deliberate; it kept the entire sweep within one overnight budget. Ulises ran the original sweep on his AMD Radeon R9700; we cross-validated key points on the 5090. The A3B no-think state-acc result agreed across hardware to within 0.07 pp, our cross-hardware comparability proof.

The tournament table gives us three takeaways. One, the Gemma 4 family decisively dominates. A4B at thirty-eight-point-six-seven percent, Gemma 4 31B at thirty-five-point-eight-six, both clear of the next-best non-Gemma model by five points or more. Two, high ROUGE-1 does NOT predict high state accuracy. Qwen3 14B leads the table on ROUGE-1 at 39.5 but ranks fifth on state acc. DeepSeek R1 14B has a high ROUGE-1 of 35.7 and ranks dead last on the pedagogical axis at eight percent. That's the empirical seed of the benchmark critique we'll get to on slide 12: surface-form metrics measure phrasing, not teaching. Three, and this is the methodological point: the tournament is an INPUT, not a verdict. The number one no-think model, A4B, was not the one we promoted to our first locked headline. A4B's smoke-mini projection sat below A3B's, and we later learned A4B carries the same schema-fallback risk that crushed Gemma 31B at full scale. So we promoted A3B (which ranks number seven in the no-think tournament) to our first full-scale locked headline. The cascade plus the tournament feed into a Bayesian decision; neither alone is the answer.
-->
...
This is where the rigor mattered most. After the cascade and the tournament, the natural next move was to promote the tournament leader's dense variant, Gemma 4 31B, to a full n=681 run. The smoke and mini had been perfect. The smoke-mini average projected the full lift at about plus-eight percentage points above A3B's locked headline. We kicked the run off overnight, expecting to wake up to a new locked headline. Twenty-one hours and forty-nine minutes of GPU time later, we got the result. 31.39 percent. Seven points below A3B. Fifteen points below the projection. That isn't a noisy outcome; that's a catastrophic miss. So we did what scientists do when the model fails: we became detectives.

The smoke-mini predictor was off by fifteen points. That has no business happening; on A3B it was accurate to within zero-point-one. So either the predictor is broken or it's missing a variable. We instrumented per-turn fallback rates, measuring how often the model failed to produce valid JSON for the fusion schema and fell back to the safety default. At full scale, 21 percent of Gemma's outputs failed the JSON schema. A3B's fallback rate on the exact same protocol was 0.91 percent, a twenty-times gap. And the small-sample runs (smoke at n=5, mini at n=25, each under 150 turns total) had encountered exactly zero fallbacks. The failure rate is a long-tail phenomenon. It only emerges at sample sizes in the hundreds. So the predictor isn't broken; it's missing a variable, and the variable is schema-fallback rate.
...
**Probe 1 — direct memorization detection.** We compared SocratTeachLLM's generated outputs character-for-character against the training corpus, and ran the same probe on a clean Gemma 31B control. The full threshold cascade:

| ROUGE-1 threshold | STL · TEST (n=288) | Gemma 31B control (n=285) |
|---|---:|---:|
| Exact match (=100%) | **4 (1.4%)** | 0 (0.0%) |
| ≥90% | 9 (3.1%) | 0 (0.0%) |
| ≥80% (near-verbatim) | 17 (5.9%) | 0 (0.0%) |
| ≥70% | 29 (10.1%) | 3 (1.1%) |
| ≥60% (high-overlap) | 61 (21.2%) | 10 (3.5%) |
| **≥50%** | **110 (38.2%)** | 46 (16.1%) |

The gap is asymmetric and grows at the strict thresholds — Gemma produces **zero** outputs at ≥80% ROUGE-1; STL produces 17. **Nearly 40% of STL's outputs share at least half their characters with its training data**, more than 2× Gemma's rate. There is no plausible non-memorization explanation for that distribution.
...
- **Memorization probe (full cascade):** STL produces 4/288 exact matches, 17/288 ≥80% near-verbatim, 61/288 ≥60% high-overlap, **110/288 ≥50% match**. Gemma 31B control on the same probe: **0/285 at any threshold above 70%**; only 46/285 at ≥50%. STL's distribution has a fat right tail Gemma simply does not have.
...
Probe one — direct memorization detection. We compared SocratTeachLLM's generated outputs character-for-character against the training corpus and ran the same probe against a clean Gemma 31B control. The table shows the full threshold cascade. Four outputs are character-for-character identical to training data, that's the top row, 1.4 percent. Seventeen are at least 80 percent ROUGE-1 overlap, the near-verbatim threshold, that's 5.9 percent. Sixty-one are at least 60 percent. And here's the number that lands: one-hundred-and-ten outputs, nearly 40 percent of all turns, share at least half their characters with the training corpus. The Gemma control on the exact same probe produces zero exact matches, zero near-verbatim, and sixteen percent at the 50 threshold — less than half STL's rate. The asymmetry grows at the strict thresholds: Gemma produces zero outputs at 80 percent or above; STL produces seventeen. STL's distribution has a fat right tail that Gemma simply does not have. There is no plausible non-memorization explanation for that.
...
We adopt **stage-balanced** because closure is the pedagogically load-bearing moment of the dialogue. It's one half of the unified-score formula on slide 16.
...
- **Single judge, not ensemble.** Defensible, reproducible, ~$15 per full evaluation. A 3-judge ensemble (Sonnet + GPT-4o + Gemini) would roughly triple the cost without obvious payoff on this benchmark.
- **Mostly third-party.** For our open-weight headlines, Sonnet judges a different family (Gemma / A3B / Qwen). For the frontier stress test (slide 11), Claude IS the teacher and Sonnet IS the judge — we accept that same-family overlap for cross-cell consistency and flag it as a known caveat rather than swap judges mid-leaderboard.
...
Why Claude Sonnet 4.6 specifically? Three reasons. First, frontier-tier capability — the judge needs to understand both Chinese pedagogy and the SocRule framework well enough to grade four axes per turn. Sonnet 4.6 does both. Second, single-judge rather than ensemble — keeps the cost down to about fifteen dollars per full evaluation; a three-judge ensemble would roughly triple that without obvious payoff on this benchmark. Third, mostly third-party — for our open-weight headlines, Sonnet judges a different model family. For the frontier stress test on slide 11 where Claude is itself the teacher, Sonnet is also the judge, and we accept that same-family overlap for cross-cell consistency rather than swap judges mid-leaderboard. It's a known caveat we flag honestly.
...
6. **Contamination proof** — two independent evidence streams: memorization probe with full ROUGE-1 cascade (STL 4/288 exact + 17/288 ≥80% + 110/288 ≥50% vs.\ Gemma 0/285 exact + 0/285 ≥80% + 46/285 ≥50%) + synthetic clean-probe (SocratTeachLLM collapses 63.4 → 32.86 stage_bal on demonstrably-unseen synthetic data).
