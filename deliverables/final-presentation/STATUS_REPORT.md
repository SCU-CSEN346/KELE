...
| Merge LoRA adapter → GGUF Q5_K_XL with **KELE-tagged filename**. Three-step pipeline: (1) `peft.merge_and_unload()` → `outputs/sft-stage2-gemma4-31b/merged/` (HF checkpoint, project-local for safety); (2) llama.cpp `convert_hf_to_gguf.py` → `outputs/sft-stage2-gemma4-31b/gemma-4-31B-kele-socratic-sft-Q5_K_XL.gguf` (verify locally first); (3) once verified, copy to `~/Documents/models/weights/gemma-4-31B-kele-socratic-sft-Q5_K_XL.gguf`. **Do NOT overwrite any existing `gemma-4-31b-it-*.gguf` in the shared models dir** — the KELE filename must coexist with base-model weights used by other projects. | ~1 h human + GPU | 2026-05-28 AM | Project-local artifact + clearly-named shared copy. Drop-in compatible with `serve_gemma4_31b_q5.sh` via `GEMMA4_31B_WEIGHT_FILE=...kele-socratic-sft-Q5_K_XL.gguf` override. |
