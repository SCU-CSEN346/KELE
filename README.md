...
> **🎯 Direct ablation against STL's own base (2026-05-29).** A within-architecture matched-pair ablation at n=681 (rows sb#1‡, sb#4‡, sb#36‡, sb#38‡ in the master leaderboard below) isolates the contamination signal at the cleanest level: SocratTeachLLM's LoRA fine-tune adds **+7.78 to +8.95** state-accuracy points but **+14.42 to +15.00** ROUGE-1 points over its own pre-trained base (`THUDM/glm-4-9b-chat`) — a **1.68×–1.85× surface-form inflation per pedagogical point**. The only variable is the LoRA adapter, and the only adapter is the one trained on SocratDataset. STL's ~15 R-1 advantage above its pre-trained base is benchmark memorization, not transferable pedagogical capability.
...
6. **🎯 STL's contamination overlay is now directly quantified (2026-05-29 — within-architecture base-model ablation).** SocratTeachLLM is a LoRA fine-tune of `THUDM/glm-4-9b-chat`. The four new rows sb#1‡, sb#4‡, sb#36‡, sb#38‡ run STL and its own pre-trained base under matched conditions at n=681 (vLLM, fewshot10, matched consultants). The LoRA fine-tune adds **+7.78 to +8.95 pp** of state accuracy but **+14.42 to +15.00 pp** of ROUGE-1 — surface-form gain is **1.68×–1.85× the pedagogical gain** at matched everything. The only variable between paired rows is the LoRA adapter, and the only adapter is the one trained on SocratDataset; this is the cleanest contamination signature in the experimental record, complementing the four prior diagnostics (surface-form rank inversion, monotonic n-gram-length scaling, cross-lingual translation, synthetic clean-probe collapse). **SocratTeachLLM consequently cannot be ranked alongside our locked open-weight integration on a clean leaderboard;** the GLM-4-9B-Chat-base rows (sb#36‡, sb#38‡) are its true pedagogical ceiling, and the ~15 R-1 gap above that ceiling is the portion of its published headline attributable to benchmark memorization rather than transferable pedagogical capability. Full proof in [`docs/SOCRATTEACHLLM_CONTAMINATION_PROOF.md`](docs/SOCRATTEACHLLM_CONTAMINATION_PROOF.md) and paper §`sec:results-critique`.
...
> **Note:** Row sb#2 (`qwen3.5 × Gemma-31B · fewshot10 · n=681`, 2026-05-26) was inserted into this table on landing. Rows labeled sb#2 through sb#142 below retain their pre-insertion rank numbers (off-by-one vs. the regenerated snapshot at `results/_orchestrator_logs/backtest_stage_balanced_2026_05_26.md`); the contents and ordering are correct. **A second batch of four ‡-suffixed insertions landed 2026-05-29 (sb#1‡, sb#4‡, sb#36‡, sb#38‡) — the GLM-4-9B-Chat-base × * cells (STL's pre-trained base, the "true ceiling" reference) and matching STL n=681 cells from the within-architecture base-model ablation campaign (see footnote 6 in the **Key findings** section above).**
...
| ⚠️ 1‡ | `qwen3.5 × SocratTeachLLM · fewshot10 · n=681` ← NEW 2026-05-29 · STL ablation upper row · contamination-driven | 3993 | 56.50 | **61.78** | — | 47.45 |
...
| ⚠️ 4‡ | `bert-fixed × SocratTeachLLM · fewshot10 · n=681` ← NEW 2026-05-29 · STL ablation lower row · contamination-driven | 3870 | 52.66 | **60.10** | — | 47.30 |
...
| 36‡ | `qwen3.5 × GLM-4-9B-Chat-base · fewshot10 · n=681` ← NEW 2026-05-29 · STL's pre-trained base · true ceiling reference (no contamination flag — base, not fine-tune) | 4220 | 48.72 | **53.37** | — | 33.03 |
| 37 | `bert × Claude-Sonnet · raw · n=50` | 260 | 45.00 | **53.32** | 7.25 | 29.10 |
| 38 | `tournament-cell-10-compressed_history` | 280 | 47.50 | **52.87** | — | 38.79 |
| 38‡ | `bert-fixed × GLM-4-9B-Chat-base · fewshot10 · n=681` ← NEW 2026-05-29 · STL's pre-trained base · true ceiling reference | 3651 | 43.71 | **52.77** | — | 32.30 |
