...
def leaf(px, py, deg, L=52, Wd=17, fill=RED):
    d = (f"M0,0 C {Wd},{-L*0.30} {Wd},{-L*0.72} 0,{-L} "
         f"C {-Wd},{-L*0.72} {-Wd},{-L*0.30} 0,0 Z")
    return (f'<path transform="translate({px:.1f},{py:.1f}) rotate({deg:.1f})" '
            f'd="{d}" fill="{fill}"/>')
...
def book():
    ...
    def lines(x0, w, count, y0=702):
        return ''.join(
            ...
            f'fill="{RED if i==0 else GREYD}"/>' for i in range(count))
    binary = ''.join(
...
def wordmark():
    ...
    line([("for ", RG), ("L", BD), ("LM ", RG), ("E", BD), ("ducators", RG)], 1022, 33)
    g.append('</g>')
    return "\n".join(g)
